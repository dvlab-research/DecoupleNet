...
def create_adversarial_dropout_mask(mask, jacobian, delta):
    ...
    # print("change_candidates: ", change_candidates.sum())
    ...
    return adv_mask.clone().detach(), None


def calculate_jacobians(h, clean_logits, head, classifier, consistency_criterion):
    ...
    h_logits = classifier(head(cnn_mask * h))
    ...
    # print("cnn_mask.grad.max(): {}, cnn_mask.grad.min(): {}, cnn_mask.grad.mean(): {}".format(cnn_mask.grad.max(), cnn_mask.grad.min(), cnn_mask.grad.mean()))
