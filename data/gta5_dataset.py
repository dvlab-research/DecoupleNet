...
class GTA5_loader(BaseDataset):
    ...
    def __init__(self, opt, logger, augmentations=None):
        ...
        max_iters = opt.num_steps * opt.batch_size * opt.world_size
        if max_iters is not None:
            if not os.path.exists("data/class_balance_ids_{}.p".format(max_iters)):
                self.label_to_file, self.file_to_label = pickle.load(open(os.path.join(self.root, "gtav_label_info.p"), "rb"))
                self.ids = []
                SUB_EPOCH_SIZE = 3000
                tmp_list = []
                ind = dict()
                ...
                for e in range(int(max_iters/SUB_EPOCH_SIZE)+1):
                    cur_class_dist = np.zeros(self.n_classes)
                    for i in range(SUB_EPOCH_SIZE):
                        ...
                        w = w/w.sum()
                        c = np.random.choice(self.n_classes, p=w)
                ...
                self.ids = [os.path.join(self.label_base_path, x) for x in tmp_list]
                with open("data/class_balance_ids_{}.p".format(max_iters), 'wb') as f:
                    pickle.dump(self.ids, f)
            else:
                with open("data/class_balance_ids_{}.p".format(max_iters), 'rb') as f:
